...
data_cleaned <- data %>% 
  mutate(
    ...
    ai1_correct_cnt = ifelse(q1ai1 == ai1_correct1, 1, 0) + ifelse(q2ai1 == ai1_correct2, 1, 0),
    ai2_correct_cnt = ifelse(q1ai2 == ai2_correct1, 1, 0) + ifelse(q2ai2 == ai2_correct2, 1, 0),
    mary_correct_cnt = ifelse(order == 1, ai1_correct_cnt, ai2_correct_cnt),
    james_correct_cnt = ifelse(order == 0, ai2_correct_cnt, ai1_correct_cnt)
...
```{r lm-diff-trust}
mod_trust = lm(ai_diff_trust ~ education, data = data_cleaned)
summary(mod_trust)
```

```{r lm-diff-relevance}
mod_rele = lm(ai_diff_relevant ~ gender + order, data = data_cleaned)
summary(mod_rele)
```
